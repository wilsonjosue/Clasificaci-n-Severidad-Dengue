...
*Resultado clave:* La diferenciación permitió a Random Forest pasar de un R² de 0.41 a 0.96 en el período de brote atípico, rompiendo el límite de extrapolación.
...
1. **Redefinición del target:** Las tres clases originales (sin signos, con signos, grave) se colapsaron a binaria (No Grave vs Grave) al detectar la "clase sándwich" que impedía la separación.
2. **Partición temporal:** Train hasta 2021, validación 2022, test 2023-2024, evitando fuga de información del futuro.
...
4. **Modelo:** XGBoost.
5. **Optimización de umbral:** Se eligió el punto de corte que maximiza el F1-score en validación (τ=0.0076), reduciendo falsas alarmas.
6. **Métricas:** Accuracy, Recall, Precision, F1, PR-AUC, matriz de confusión.

*Resultado clave:* El PR-AUC de apenas 0.0056 (cercano al azar) demuestra estadísticamente que las variables disponibles no contienen información suficiente para discriminar la gravedad individual. Las variables más relevantes fueron las geográficas y temporales, no las clínicas.
...
1. **Agregación y filtrado:** Se crearon perfiles por departamento con variables como edad media, tasas de severidad, semana pico. Se excluyeron regiones con <100 casos históricos (ruido administrativo).
2. **Selección de K:** Método del codo (inercia) y Coeficiente de Silhouette determinaron K=2 como óptimo.
3. **Algoritmos comparados:** K-Means y Clustering Jerárquico, evaluados con Silhouette y Davies-Bouldin para garantizar estabilidad.
4. **Visualización:** PCA (70.7% de varianza explicada en 2D) con identificación de Piura como outlier extremo.
...
| Clasificación de gravedad | XGBoost + calibración bayesiana | PR-AUC = 0.0056 | Los datos demográficos no predicen la gravedad |
...
- **Clasificación de gravedad:** Con las variables abiertas actuales no es posible construir un clasificador clínico útil para dengue grave. Es indispensable incluir datos de laboratorio, serotipo, comorbilidades y signos vitales para ese fin.
...
- **Valor de los datos abiertos:** Los datos del MINSA permiten construir sistemas de monitoreo y pronóstico de carga de enfermedad, pero son limitados para el análisis de riesgo individual. Complementarlos con fuentes climáticas y clínicas potenciará significativamente las herramientas de salud pública.

*Nota: Los datos originales no se incluyen; pueden descargarse desde la fuente oficial.*
