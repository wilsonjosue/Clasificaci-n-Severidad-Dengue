%==============================================================================
%  SIMBIG 2026 -- Long paper (Springer LNCS / CCIS format)
%  Dengue Case Forecasting in Peru Using Time-Series Machine Learning Models
%
%  DOCUMENT CLASS: Springer "llncs" (single column), required by SIMBIG.
%  Compile on Overleaf; the llncs.cls file is available there by default.
%  FIGURES: upload the three PNGs preserving the folder
%           notebooksV2/imagenes/01_Regresion/ (or replace the existing ones,
%           same file names). The bibliography is a manual thebibliography for
%           portability (no external .bib file needed).
%==============================================================================
...
\usepackage{multirow}
\usepackage{graphicx}
\graphicspath{{Clasificaci-n-Severidad-Dengue/notebooksV2/imagenes/}{notebooksV2/imagenes/}{./}}
...
\title{Dengue Case Forecasting in Peru Using Time-Series Machine Learning Models}
\titlerunning{Dengue Case Forecasting in Peru Using ML Models}

\author{Wilson J.\ Turpo Huanca \and
        Jhastyn J.\ Payehuanca Riquelme \and
        Jeanpiero S.\ Huamani Condori}
\authorrunning{W.\ Turpo Huanca et al.}

\institute{Universidad Nacional de San Agust\'in de Arequipa (UNSA),
  Arequipa, Peru\\
  \email{\{wturpoh, jpayehuancar, jhuamanicond\}@unsa.edu.pe}\\
  Advisor: Carlos E.\ Atencio Torres, \email{catencio@unsa.edu.pe}}
...
\begin{abstract}
  ...
  for each target week we \emph{add $W$ columns holding the case counts of the $W$
  immediately preceding weeks} (weekly lag features; the window spans whole calendar
  weeks, not the days inside a week). We explicitly separate this \emph{input window}~$W$ (past weeks used as
  ...
  elbow-type criteria. We compare six machine learning techniques for regression --- Ridge,
  Lasso, Multilayer Perceptron, Random Forest, Gradient Boosting and an LSTM network.
  Surprisingly, the best model is a regularized
  ...
  \keywords{Dengue \and Case forecasting \and Time series \and Regression \and
  Machine learning \and Early warning \and Public health \and Peru}
\end{abstract}
...
to \emph{Cyclone Yaku}, which brought intense rainfall and coastal flooding to northern and
central Peru in March~2023 and created favourable breeding conditions for \emph{Aedes
aegypti}~\cite{yaku2023}. Accurate short-term forecasts of weekly case counts support
...
(i)~the almost systematic \emph{use of exogenous covariates}\footnote{We use \emph{exogenous
variable} (or covariate) in the standard time-series / econometric sense ---as in
ARMAX-type models, where the ``X'' stands for eXogenous~\cite{boxjenkins,hyndmanFPP}--- to
denote any predictor \emph{external} to the series being forecast, i.e.\ not derived from
its own past values (e.g.\ temperature, rainfall, human mobility). This is the opposite of an
\emph{endogenous} feature, such as the lagged case counts our model exclusively uses (\S3.4).
Not every cited work uses this exact word, but all of them rely on such external data
sources.} (climate and, increasingly,
...
an LSTM on the wavelet-denoised residuals, and an SI-SIR (susceptible--infected /
susceptible--infected--recovered) compartmental model. Their
ensemble forecasts with good precision up to about \textbf{60 weeks} ahead
(mean absolute percentage error, MAPE~$=13.97$)~\cite{hyndman2006}, far beyond the usual
few weeks, but it relies on climate variables and a
...
\begin{table}[t]
\centering
\caption{Summary of recent dengue-forecasting literature (last three years). Column
``Exog.\ vars.'' lists the \emph{external} (non-case) data sources each model consumes
(see footnote~1 for the definition of ``exogenous''). All reviewed works rely on such
exogenous covariates (mainly climate). In contrast, our model is built \emph{exclusively}
from the target series' own past values ---i.e.\ previous weekly case counts, which we
call \emph{case history}--- with no external data source at all (\S3.2--3.4).}
\label{tab:sota}
\resizebox{\textwidth}{!}{%
...
Abiertos}, Peru)~\cite{minsa}, covering 2000--2024 with \textbf{1{,}029{,}421}
individual case records and 14 columns
(department, province, district, locality, disease type, year, epidemiological week,
ICD-10 code, health directorate, geocode, age, age unit, sex). A \textbf{record} is one row
of the raw dataset, i.e.\ \emph{one individually notified dengue case}, timestamped by
epidemiological week and year and tagged with the patient's location (department, province,
district) and demographics (age, sex). Identifier and
...
\paragraph{Scope: national, not regional.} Department, province and DIRESA are used only to
\emph{aggregate} records into a single \textbf{national} weekly count (\S3.2); they are
\emph{not} inputs to the forecasting model, and the model does \emph{not} produce
region-specific forecasts. Concretely, given the national case counts of the $W=3$ most
recent weeks, the model outputs the national case count for the following week (i.e.\ week
$t{+}1$ from weeks $t{-}2,t{-}1,t$); it does not take a region as input, nor a 4-week
window. Region-level forecasting is discussed as future work in Section~6.

\begin{table}[t]
\centering\footnotesize
\caption{Variable dictionary and the role each field plays in this study (regression only;
a companion severity-classification study uses disease type / ICD-10 separately and is out
of scope here). \textbf{Aggregation key}: used only to \emph{group} individual case records
into the weekly national count $\text{cases}_t$ (\S3.2); it never enters the model as a
numeric predictor by itself. \textbf{Examined in EDA}: aggregated at the weekly level and
correlated against $\text{cases}_t$ (Fig.~\ref{fig:corr}); given a near-zero correlation, it
was \emph{not} retained as a feature. \textbf{Discarded}: identifier, high-cardinality, or
irrelevant to the weekly count. The final regression model is \emph{univariate}: its only
inputs are lagged values of $\text{cases}_t$ itself (\S3.4).}
\label{tab:dict}
\begin{tabular}{@{}llp{3.6cm}@{}}
\toprule
Variable & Type & Role in this study \\
\midrule
department / province & categorical & Aggregation key only \\
district / locality & categorical/text & Discarded (high cardinality) \\
disease type & categorical & Discarded (out of scope; used only in a companion classification study) \\
year & numeric & Aggregation key: defines the week index of $\text{cases}_t$ \\
week (1--53) & numeric & Aggregation key: defines the week index of $\text{cases}_t$ \\
diagnostic (ICD-10) & categorical & Discarded (redundant with disease type) \\
diresa & categorical & Aggregation key only \\
geocode / loc.\ code & ID & Discarded (identifier, invalid as feature) \\
age / age unit & numeric/cat. & Examined in EDA ($r\approx0.06$ with $\text{cases}_t$); not used as a feature \\
sex & categorical & Examined in EDA ($r\approx0.11$ with $\text{cases}_t$); not used as a feature \\
\bottomrule
\end{tabular}
\end{table}
...
The raw dataset exposes fourteen raw fields ---department, province, district, locality,
disease type, year, epidemiological week, ICD-10 diagnostic code, health directorate
(DIRESA), geocode, locality code, age, age unit and sex (Table~\ref{tab:dict})---, of
which more than ten qualify as candidate explanatory variables. A relation (correlation)
analysis, however, shows that none of them carries usable signal
...
lag features, the counts of the $N$ immediately preceding weeks. Recasting a series as a
supervised problem through lagged (window) features and then scanning increasing window
lengths is standard practice in time-series forecasting~\cite{hyndmanFPP,boxjenkins}; we
start from a small window ($W=3$) because the partial autocorrelation of the series cuts
off after lag~3 (\S\ref{sec:window}), the classical Box--Jenkins criterion for selecting
the order of an autoregressive model~\cite{boxjenkins}. On top of this single
...
comparison is fair and reproducible. The whole pipeline is implemented in a single public
notebook (\texttt{01\_Regresion}), from which every figure reported below is exported.
...
of the series, i.e.\ \textbf{2020 (week~12) through 2024 (week~52), $\approx$4.8 years}),
avoiding random splits that would leak future information. This test window contains both
the 2023 and 2024 upsurges (\S4.1), which is why we describe it below as the
out-of-distribution period. We compare
...
%       Replace them with the actual PNG exports from notebook 01_Regresion if desired:
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{01_Regresion/output_1_1.png}
...
$W=3$ lag features. This combined ACF/PACF diagnostic is the classical Box--Jenkins
visualization~\cite{boxjenkins}, reproduced with the same side-by-side layout and shaded
95\% significance bands in Hyndman \& Athanasopoulos~\cite{hyndmanFPP} (Chs.~2.9 and~9.5,
\emph{Forecasting: Principles and Practice}, freely available online); the plots were
generated with the \texttt{statsmodels} library~\cite{statsmodels}
(\texttt{plot\_acf}/\texttt{plot\_pacf}), whose own documentation gallery shows figures in
this exact style.}
\label{fig:acf}
\end{figure}
...
classical Box--Jenkins terms~\cite{boxjenkins} this points to an AR(3)-like structure, which is exactly the
...
\begin{table}[t]
\centering\footnotesize
\caption{Model comparison at $W=3$, $H=1$ (temporal test on 2020--2024, $\approx$4.8 years,
which includes the 2023 and 2024 upsurges).
...
predictions on the temporal test period (2020--2024), where two distinct explosive
outbreaks push the series far above any historical level: a first, smaller upsurge in 2023
(week~20, 19{,}269 cases; green dash-dotted line) associated with \emph{Cyclone
Yaku}~\cite{yaku2023}, and a second, \textbf{larger and separate} outbreak in 2024 (week~14,
21{,}628 cases; red dotted line), the highest value in the entire 25-year series. The figure
conveys two messages. First, the
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{01_Regresion/output_1_3.png}
\caption{One-week-ahead forecasts over the temporal test period (2020--2024, $\approx$4.8
years), obtained in notebook \texttt{01\_Regresion}. The black curve is the observed MINSA
weekly count; the Ridge regression (via differencing) and the na\"ive persistence baseline
closely follow both out-of-distribution outbreaks: the smaller, first upsurge in 2023
(left bump; green dash-dotted line) associated with \emph{Cyclone Yaku}, and the larger,
second and unrelated outbreak in 2024 (right bump; red dotted line), the highest weekly
count in the 25-year series. Modeling the growth rate lets even linear and tree models
extrapolate beyond the historical range, supporting the near-linear, strongly
...
\begin{itemize}\itemsep1pt
  ...
  brings no benefit. Aggregated at the national weekly level, the series is moreover
  comparatively smooth ---dominated by broad seasonal waves rather than sharp week-to-week
  spikes (Fig.~\ref{fig:forecast})--- so a linear mapping between consecutive weeks fits it
  well.
...
\bibitem{tft2026}
Pillay, M.T., Le, M.T.Q., Takamatsu, Y., Phong, T.V., Kgalane, N., Minakawa, N.:
...
\url{https://doi.org/10.3390/ijerph23010075}

\bibitem{minsa}
Ministerio de Salud del Per\'u (MINSA), Centro Nacional de Epidemiolog\'ia, Prevenci\'on y
Control de Enfermedades: Vigilancia epidemiol\'ogica de dengue. Plataforma Nacional de
Datos Abiertos, Lima, Peru (2024).
...
Time Series Analysis: Forecasting and Control, 5th edn. Wiley, Hoboken, NJ (2015).

\bibitem{hyndmanFPP}
Hyndman, R.J., Athanasopoulos, G.:
Forecasting: Principles and Practice, 3rd edn. OTexts, Melbourne (2021).
\url{https://otexts.com/fpp3/}

\bibitem{statsmodels}
Seabold, S., Perktold, J.:
statsmodels: Econometric and statistical modeling with Python. In: Proceedings of the
9th Python in Science Conference (SciPy 2010), pp.~92--96 (2010).
...
\textbf{22}(4), 679--688 (2006). \url{https://doi.org/10.1016/j.ijforecast.2006.03.001}

\bibitem{yaku2023}
Munayco, C.V., Valderrama Rosales, B.Y., Mateo Lizarbe, S.Y., Yon Fabian, C.R., Pe\~na
S\'anchez, R., V\'asquez S\'anchez, C.H., Garc\'ia, M.P., Padilla-Rojas, C., Su\'arez, V.,
S\'anchez-Gonz\'alez, L., Jones, F.K., Kohatsu, L., Adams, L.E., Morgan, J., Paz-Bailey, G.:
Notes from the field: dengue outbreak --- Peru, 2023. MMWR Morbidity and Mortality Weekly
Report \textbf{73}(4), 86--88 (2024). \url{https://doi.org/10.15585/mmwr.mm7304a4}
\end{thebibliography}
