...
%  Dengue Case Forecasting in Peru Using
...
\title{\vspace{-1.0cm}\textbf{Dengue Case Forecasting in Peru Using\\
Time-Series Machine Learning Models}}
...
\author{
  ...
  \small \texttt{catencio@unsa.edu.pe}\\
}
\date{}
...
\begin{table}[t]
\centering\footnotesize
\caption{Variable dictionary. \checkmark{}~candidate examined; \ding{55}~identifier / invalid.
No categorical field is ordinally encoded: geographic fields are used only to
\emph{aggregate} records into the weekly count, and the final model is purely univariate
(lags of the count). Candidate variables marked ``EDA only'' were inspected in the
correlation analysis but not fed to the model.}
\label{tab:dict}
\begin{tabular}{@{}lll@{}}
\toprule
Variable & Type & Feature? \\
\midrule
department & categorical & \checkmark{} (aggregation only) \\
province & categorical & \checkmark{} (aggregation only) \\
...
diresa & categorical & \checkmark{} (aggregation only) \\
geocode / loc.\ code & ID & \ding{55}\ invalid \\
age / age unit & numeric/cat. & \checkmark{} (normalized, EDA only) \\
sex & categorical & \checkmark{} (binary, EDA only) \\
\bottomrule
\end{tabular}
\end{table}
...
---including the peak that followed \emph{Cyclone Yaku} in early 2023~\cite{yaku2023}
(vertical dotted line)--- push the
...
two out-of-distribution outbreaks, including the exponential peak that followed
\emph{Cyclone Yaku} (dotted vertical line). Modeling the growth rate lets even linear and tree models extrapolate beyond
